...
import type { Room } from "../hooks/useMasterData";

const schema = z.object({
  name: z.string().min(1, "Nome é obrigatório").max(100),
  capacity: z.coerce.number().min(1, "Mínimo 1 pessoa").max(100).optional(),
  is_active: z.boolean(),
});
...
interface Props {
  ...
  room: Room | null;
  onSave: (data: Partial<Room>) => Promise<boolean>;
  userId: string;
}

export function RoomFormModal({ open, onOpenChange, room, onSave, userId }: Props) {
